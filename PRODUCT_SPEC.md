...
| Modality Library                        | ✅ Done | Shipped 2026-03-09 (Pipeline 63): 8 system set structure modalities (Straight Sets, Down Sets, Eccentrics, Giant Sets, Myo-reps, Drop Sets, Supersets, Occlusion) with volume multipliers. Configurable guardrail engine. A5 generator integration. Session volume summaries per muscle group.                                                                                                                                                                                      |
| Progression Engine                      | ✅ Done | Shipped 2026-03-09 (Pipeline 64): 5 progression profiles (Staircase Percent, Rep Staircase, Double Progression, Linear, Wave-by-Month). Deterministic evaluator per type. Gap detection (>14 days → deload), consecutive failure detection, deload scheduling. ProgressionEvent audit trail with DecisionLog. Plan-level default + slot-level override. CRUD API + 4 PlanSlot actions (next-prescription, apply-progression, progression-readiness, progression-history). 73 tests. |
| Session Runner (Backend)                | ✅ Done | Shipped 2026-03-09 (Pipeline 65): ActiveSession + ActiveSetLog models. Session lifecycle: start, log-set, skip-set, complete, abandon. Auto-creates LiftSetLog on completion. Progression engine integration on session complete. Rest timer service with slot_role defaults + modality overrides. Stale session auto-abandon (>4hr). select_for_update on all mutations. Role enforcement (trainee-only). IDOR protection via plan ownership chain. 48 tests.                      |
...
| Dual-mode program builder               | ✅ Done | Shipped 2026-03-19: Quick Build (expanded brief → full pipeline → plan + per-step "why it fits" explanations) and Advanced Builder (9-step wizard: brief → length → split → skeleton → roles → structures → exercises → swaps → progression → publish, with Why panels, alternatives, and overrides at every layer). 4 new API endpoints, 13 backend tests, 3 mobile screens.                                                                                                       |
